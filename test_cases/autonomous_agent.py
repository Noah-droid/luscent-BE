...
4. SCHEMA OBSESSION: Before calling any API, check its 'request_body' field in the AVAILABLE API ENDPOINTS list. Match its keys and casing EXACTLY. Also check 'auth_type' and 'headers' per endpoint.
   - CRITICAL: NEVER copy encrypted, encoded, or hashed values from the request_body schema. Those are stored examples, not real data. Generate FRESH, PLAIN-TEXT test data for every field.
   - ENCRYPTED SCHEMA DETECTION: If you see values like "gAAAAAB..." in the schema, the schema is encrypted. Ignore the values and use FIELD NAMES + common sense to generate data. For example: "email" → real email, "password" → "TestPass123!", "first_name" → "John", "role" → "user", "dob" → "1990-05-15", "nin" → "12345678901". For example: use a real date like "1990-05-15" for dob, a numeric string like "12345678901" for nin, "user" for role, "testuser_123@example.com" for email (or the AGENT_EMAIL if available), etc. If a field value looks like "gAAAAAB..." or starts with random long strings, it is encrypted — generate a fresh plain-text value instead.
   - ENCRYPTED SCHEMA RULE: If ALL schema values appear encrypted (gAAAAAB...), ignore the schema values entirely. Use the FIELD NAMES to infer what data to send: "email" → real email, "password" → strong password, "first_name"/"last_name" → realistic names, "dob" → date of birth like "1990-05-15", "nin" → numeric ID string like "12345678901", "role" → try "user" or "admin".
